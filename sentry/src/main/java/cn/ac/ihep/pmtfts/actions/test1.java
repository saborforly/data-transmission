...
public class test1 extends SuspendableImpl implements RealizedAction {
	/**
	...
	public test1(final Action action, final Boolean suspend, final Context context) {
		super(suspend);
	...
	/*
	 public static void main(String args[]) throws JAXBException{
		...
		File semphoreFile=new File("1.sem");
		JAXBContext content = JAXBContext.newInstance(new Class[] { Configuration.class });
		Unmarshaller unmarshaller = content.createUnmarshaller();
		configuration=(Configuration)unmarshaller.unmarshal(semphoreFile);
		List<Action> actions= configuration.getActions();
		for( int i = 0 ; i < actions.size() ; i++) {//内部不锁定，效率最高，但在多线程要考虑并发操作的问题。
		    System.out.println(actions.get(i));
		    Action action=actions.get(i);
		    String base = action.getParameter("base");
	 ...
	 */
	 public static void main(String args[]) throws JAXBException, ParseException{
		 ...
		 String lastDateTime = format2.format("2018-08-10 13:02:27");
